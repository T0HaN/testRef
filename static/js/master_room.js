...
async function activateScene(sceneId) { try { const response = await fetch(`/api/room/{{ room.id }}/scene/${sceneId}/activate`, { method: 'POST' }); const data = await response.json(); if (data.status === 'ok') { if (roomWs && roomWs.readyState === WebSocket.OPEN) { roomWs.send(JSON.stringify({ type: 'tokens_clear' })); } loadScenes(); } } catch (e) { console.error(e); } }
...
async function loadScenes() { try { const response = await fetch(`/api/room/{{ room.id }}/scenes`); const data = await response.json(); if (data.status === 'ok') renderScenesList(data.scenes); } catch (e) { console.error(e); } }
...
async function uploadNewScene(inputElement) { const file = inputElement.files[0]; if (!file) return; const nameInput = document.getElementById('newSceneName'); const name = nameInput.value.trim() || 'Новая сцена'; const btn = inputElement.nextElementSibling; const originalText = btn.innerHTML; btn.innerHTML = '⏳ Обработка...'; btn.disabled = true; const img = new Image(); img.onload = async () => { const formData = new FormData(); formData.append('file', file); formData.append('name', name); formData.append('width', img.naturalWidth); formData.append('height', img.naturalHeight); try { const response = await fetch(`/api/room/{{ room.id }}/scene/upload`, { method: 'POST', body: formData }); const data = await response.json(); if (data.status === 'ok') { nameInput.value = ''; loadScenes(); } else alert('Ошибка: ' + data.message); } catch (e) { console.error(e); alert('Ошибка при загрузке файла'); } finally { btn.innerHTML = originalText; btn.disabled = false; inputElement.value = ''; } }; img.src = URL.createObjectURL(file); }
async function deleteScene(sceneId) { if (!confirm("Удалить эту сцену? Это действие нельзя отменить.")) return; try { const response = await fetch(`/api/room/{{ room.id }}/scene/${sceneId}`, { method: 'DELETE' }); const data = await response.json(); if (data.status === 'ok') loadScenes(); } catch (e) { console.error(e); } }
...
async function sendReward(type) {
    ...
    try {
        const resp = await fetch('/master/room/{{ room.id }}/reward', {
            method: 'POST',
